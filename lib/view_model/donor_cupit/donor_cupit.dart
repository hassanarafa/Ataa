...
import '../../UI/view/events_screen/Order_data.dart';
...
class OrderCubit extends Cubit<OrderState> {
  OrderCubit(OrderState initialState) : super(InitialOrderState());
...
/*-----------------------pay_type-----------------------------*/
  ...
  Future<List<BankBranchItems>> fetchBankBranch() {
    print("Org Bank Account");
    return DioHelpers.getData(url: "bank/" + selectbankCode.toString())
        .then((value) {
...
/*----------------------- Post Donnation transfer -----------------------------*/
  ...
  postClient(
    context, {
    required client_nationality_id,
    required name,
    required address,
    required project_name,
    required age,
    required the_expected_amount_of_financing,
    required expected_profit,
    required required_bank,

    //    required events
  }) {
    return DioHelpers.postData(url: "client/", data: {
      "client_nationality_id": client_nationality_id,
      "name": name,
      "address": address,
      "project_name": project_name,
      "age": age,
      "the_expected_amount_of_financing": the_expected_amount_of_financing,
      "required_bank": required_bank,
      "expected_profit": expected_profit,
    }).then((value) {
      print(value);

      var donorDateModel = ClientDatoModel.fromJson(value.data);
      print("*");

      Navigator.push(context, MaterialPageRoute(builder: (c) {
        return orderData(
          clientDatoModel: donorDateModel,
          //     eventsModel: events,
        );
      }));
      showToast("تم تسجيل بياناتك بنجاح",
          position: ToastPosition.bottom, duration: Duration(seconds: 4));
      print(value);
    }).catchError((e) {
      print(e.toString());
      print(DateFormat('dd-MM-yyyy').format(
          BlocProvider.of<OrderCubit>(context).birth_date ?? DateTime.now()));
    });
  }
}
